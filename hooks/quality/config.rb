...
module Quality
  ...
  OWN_SPEC = <<~MSG.chomp
    Shrink that file only this turn:
    ...
    4. Keep coverage. Run the affected spec files. Pre-existing size is not an excuse to skip.
  ...
  OTHER_SPEC = <<~MSG.chomp
    Review each one for redundancy: examples covering a path that is already tested, setup copied from example to example instead of a shared let or before, unused lets, stubs and includes, fixtures the code under test never reads. Remove only what goes away without losing a check - keep the edge cases. Run these files after editing. If there is nothing to cut, say so and leave the file alone.
  MSG
  MODULE_SHRINK = <<~MSG.chomp
    ...
    5. Keep behavior and public API stable. Pre-existing size is not an excuse to skip.
